...
Plot3Copy <- function() {
    windows()
    PlotChart3()
    dev.copy(png, file = "plot3_copy.png")
    dev.off()
}

#
#Function: PlotChart3 - This code prepares the chart
PlotChart3 <- function() {
    plot(fdata2$DateTime, fdata2$Sub_metering_1,  type = "l", main = "" , 
         ylab="Energy sub metering",
         xlab="")
    
    with(fdata2, plot(DateTime, Sub_metering_1, main = "" , 
                      ylab="Energy sub metering",
                      xlab="", type = "l"))
    with(fdata2, lines(DateTime, Sub_metering_2, col="red"))
    with(fdata2, lines(DateTime, Sub_metering_3, col="blue"))
    legend("topright", lty = 1, lwd = 1, col=c("black", "red", "blue"), 
           legend =  c(names(fdata2[7:9])))
}

#execute the functions to create output
Plot3PNG()  #creates output that will be posetd to GITHUB repo
Plot3Copy() 
